...
export class Adaptive {
    ...
    public SetPeriod(period: number) {
        if (this.loopNumber != null) clearInterval(this.loopNumber);

        this.loopNumber = setInterval(
            this.getConnectionStats.bind(this),
            period
        );
    }
    ...
    constructor(conn: RTCPeerConnection, callback: MetricCallback) {
        this.last_timestamp = {
            audio: 0,
            video: 0,
            network: 0
    ...
    filterVideo(report: RTCStatsReport): VideoMetrics {
        ...
        report.forEach((val, key) => {
            if (val['type'] == 'inbound-rtp' && val['kind'] == 'video') {
                ...
                ret.totalSquaredInterFrameDelay =
                    val['totalSquaredInterFrameDelay'];
    ...
    private async getConnectionStats() {
        ...
        if (
            network != null &&
            network.timestamp != this.last_timestamp.network
        ) {
